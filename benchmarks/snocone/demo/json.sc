// json.sc (Snocone BENCHMARK) - JSON DE-SERIALIZER: RFC-8259 text -> native Snocone data structures.
// Replicates the LOGIC of benchmarks/snobol4/demo/json.sno statement for statement:
// the JSON(N) kernel, the *BENCH marker, and the demo's own driver below unchanged.
...
//------------------------------------------------------------------------------
// JSON(N) -- deserialize the slurped text N times and report the node census.
// Every counter and both stacks are reset per pass, so each pass does the same
// work and the census is the census of ONE parse, not of N accumulated ones.
// N=1 is what the standalone driver below leaves unused; the timed twin built by
// scripts/bench_wrap.sh asks for many.
//------------------------------------------------------------------------------
function JSON(N) {
    ZI      =   1;
ZBL:
    ...
    JSON    =   nObj '/' nArr '/' nStr '/' nInt '/' nReal '/' nBool '/' nNull '/' maxdep;
    return;
ZBAD:
    JSON    =   'Pattern match failed';
    return;
}
//------------------------------------------------------------------------------
