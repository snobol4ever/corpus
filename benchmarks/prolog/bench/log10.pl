...
% ⛔ Timestamps go to user_error so stdout stays BYTE-COMPARABLE and log10.expected still verifies.
...
main :-
    ...
    format(user_error, "BENCH kernel=log10 work_us=~w work_ms=~w~n", [W, WM]).
