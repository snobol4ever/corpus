...
% *BENCH kernel=derive -- PRISTINE KERNEL (CEO-567): the computation and nothing else.
...
% real standalone program whose stdout is graded byte-for-byte against derive.expected (oracle-cut).
:- initialization(main).
bench_work(Res) :-
