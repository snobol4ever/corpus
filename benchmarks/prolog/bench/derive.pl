...
% ⛔ Timestamps go to user_error so stdout stays BYTE-COMPARABLE and derive.expected still verifies.
...
main :-
    ...
    format(user_error, "BENCH kernel=derive work_us=~w work_ms=~w~n", [W, WM]).
