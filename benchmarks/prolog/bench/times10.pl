...
% ⛔ Timestamps go to user_error so stdout stays BYTE-COMPARABLE and times10.expected still verifies.
...
main :-
    ...
    format(user_error, "BENCH kernel=times10 work_us=~w work_ms=~w~n", [W, WM]).
