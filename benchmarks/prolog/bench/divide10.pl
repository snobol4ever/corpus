% *BENCH kernel=divide10 -- PRISTINE KERNEL (CEO-567): the computation and nothing else.
...
% real standalone program whose stdout is graded byte-for-byte against divide10.expected (oracle-cut).
:- initialization(main).
bench_work(Res) :-
