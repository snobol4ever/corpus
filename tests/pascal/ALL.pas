...
{----------------------------------------------- 75 program_procedure_nested_13}
program t(output);
var x: integer;
...
{----------------------------------------------- 84 program_procedure_nested_22}
program nestvar;
var x: integer;
...
{---------------------------------------- 87 ladder__rung05_proc_func_no_params}
...
{----------------------------------------------- 88 program_procedure_nested_11}
program varframe;
var g : integer;
  procedure bump(var n : integer);
  begin
...
{----------------------------------------------- 89 program_procedure_nested_12}
program vartrans;
var g : integer;
...
{----------------------------------------------- 90 program_procedure_nested_18}
program nestedtest;
var g : integer;
  procedure outer;
  var x : integer;
...
{----------------------------------------------- 91 program_procedure_nested_25}
program varframe;
var g : integer;
  procedure bump(var n : integer);
  begin
...
{----------------------------------------------- 92 program_procedure_nested_26}
program vartrans;
var g : integer;
  procedure inner(var z : integer);
  begin
...
{----------------------------------------------- 95 program_procedure_nested_20}
program nestrec;
var g : integer;
...
{----------------------------------------------- 97 program_procedure_nested_17}
program nestcount;
var g : integer;
  procedure counter;
  var c : integer;
...
{---------------------------------------------- 100 program_procedure_nested_23}
program nestvar2;
var x: integer;
procedure outer(var a: integer);
  procedure middle;
...
{----------------------------------------------- 101 program_procedure_nested_9}
program nestvar2;
var x: integer;
procedure outer(var a: integer);
  procedure middle;
...
{---------------------------------------------- 102 program_procedure_nested_10}
program nestvar3;
var x, y: integer;
...
{---------------------------------------------- 103 program_procedure_nested_24}
program nestvar3;
var x, y: integer;
procedure swapvals(var a, b: integer);
  procedure doswap;
...
{---------------------------------------------- 104 program_procedure_nested_21}
program nestshadow;
var g : integer;
  procedure outer;
  var x : integer;
...
{----------------------------------------------- 105 program_procedure_nested_7}
program nestshadow;
var g : integer;
  procedure outer;
  var x : integer;
...
{---------------------------------------------- 106 program_procedure_nested_16}
program nest2;
var g : integer;
  procedure a;
  var x : integer;
...
{----------------------------------------------- 107 program_procedure_nested_2}
program nest2;
var g : integer;
  procedure a;
  var x : integer;
...
{----------------------------------------------- 108 program_procedure_nested_1}
program deep5;
var g: integer;
procedure p1;
var a: integer;
...
{---------------------------------------------- 109 program_procedure_nested_15}
program deep5;
var g: integer;
procedure p1;
var a: integer;
...
{---------------------------------------------- 110 program_procedure_nested_14}
...
{---------------------------------------------- 111 program_procedure_record_11}
program t;
...
{----------------------------------------------- 112 program_procedure_record_4}
program t;
type r = record a, b: integer end;
var g: r;
procedure c(var f: r);
...
{-------------------------------------------- 114 program_record_case_replace_2}
program vrec1;
...
{----------------------------------------------- 116 program_procedure_record_7}
program recparam;
...
{---------------------------------------------- 118 program_procedure_record_10}
program t;
type r = record a, b: integer end;
var g: r;
procedure c(var f: r);
...
{----------------------------------------------- 119 program_procedure_record_3}
program t;
type r = record a, b: integer end;
var g: r;
procedure c(var f: r);
...
{-------------------------------------------- 122 program_record_case_replace_4}
program vrec4;
...
{---------------------------------------------- 124 program_procedure_forward_1}
program forward1;
var x: integer;
...
{---------------------------------------------- 125 program_procedure_forward_2}
program forward1;
var x: integer;
procedure second; forward;
procedure first;
...
{----------------------------------------------- 126 program_procedure_record_1}
...
{----------------------------------------------- 127 program_procedure_record_8}
program aggloc2(output);
type rec = record typtr: integer; kind: integer end;
var gattr: rec; depth: integer;
  procedure selector;
...
{-------------------------------------------- 129 program_record_case_replace_3}
program vrec2;
type valu = record case b: boolean of
      true: (ival: integer);
      false: (valp: integer)
...
{-------------------------------------------- 130 program_record_case_replace_1}
...
begin p1 end.
{--------------------------------------------- 133 program_while_goto_replace_1}
program goto2;
label 1, 5;
...
{--------------------------------------------- 134 program_while_goto_replace_2}
program goto2;
label 1, 5;
var i, n, total: integer;
begin
...
{------------------------------------------- 138 program_function_for_replace_1}
(*$c+,d+,l-,t-*)
...
{------------------------------------------- 139 program_function_for_replace_2}
(*$c+,d+,l-,t-*)
(* recursion on the p-machine: fact and fib *)
program recursion(output);
var
...
{----------------------------------------------- 152 program_procedure_record_2}
...
{----------------------------------------------- 153 program_procedure_record_9}
program rec_local_collision;
type rec = record f: integer end;
var g: integer;
procedure inner;
...
{-------------------------------------------- 157 program_case_repeat_replace_1}
...
{-------------------------------------------- 158 program_case_repeat_replace_2}
(* pb40: KNOWN FAIL (session 40) - no .ref on purpose; gate skips it.
   Lowering bug: inside a case arm, an if/else whose ELSE contains a loop,
   followed by a for-loop, wires the for's exit edge to the preceding IR_IF
   marker whose gamma points back at the for-init -> infinite re-init cycle
...
{------------------------------------- 225 program_function_procedure_replace_3}
...
{------------------------------------------- 226 program_record_array_replace_1}
program t;
type dt = (ci, cc);
     r = record case dt of
       ci: (vi: integer);
...
{--------------------------------------------- 227 program_function_procedure_1}
program arr2dtype;
...
{--------------------------------------------- 228 program_function_procedure_6}
program arr2dtype;
type mat = array[0..1, 0..1] of integer;
var m: mat; x: integer;
procedure setmat(var a: mat);
...
{----------------------------------------------- 230 program_procedure_record_6}
program ptr8(output);
type link = ^node; node = record val : integer; next : link end;
var head : link;
...
{------------------------------------- 231 program_function_procedure_replace_2}
...
{--------------------------------------- 232 program_procedure_record_replace_1}
program w1;
type valu = record case b: boolean of
        true: (ival: integer);
        false: (valp: integer)
...
{--------------------------------------- 233 program_procedure_record_replace_2}
program w1;
type valu = record case b: boolean of
        true: (ival: integer);
        false: (valp: integer)
...
{---------------------------------------- 236 program_procedure_array_replace_1}
...
{---------------------------------------- 237 program_procedure_array_replace_6}
{ Regression probe for the in-process binary emitter's forward-reference
  patch table (BB_PATCH_MAX). A single procedure with many statements leaves
  one pending forward-ref patch per statement (the chain jumps to the proc's
  gamma/omega ports, which resolve only at proc end). With the old cap of 512
...
{------------------------------------------ 238 program_record_packed_replace_1}
program mir3(output);
type
...
{---------------------------------------- 241 program_procedure_array_replace_3}
...
{---------------------------------------- 242 program_procedure_array_replace_8}
(* PB-32: minimal programme->block->body chain, no decls
   mirrors pcom token flow for "program x; begin end." 
   token stream: period(progsy-done)->beginsy->endsy->period *)
program pb32(output);
...
{--------------------------------------- 243 program_procedure_array_replace_10}
...
{---------------------------------------- 244 program_procedure_array_replace_5}
(* PB-34: Test complex repeat-until with eof()-like sentinel
   Simulates block()'s outer repeat: until (sy in statbegsys) or done
   where "done" is an integer boolean flag (like eof check)
   AND block's body repeat: until (sy=fsy) or (sy in blockbegsys) or done *)
...
{---------------------------------------- 245 program_procedure_array_replace_4}
...
{---------------------------------------- 246 program_procedure_array_replace_9}
(* PB-33: exact "program x; begin end." chain
   When block_sim is called: sy=beginsy(31)
   insymbol -> sy=endsy(39)
   body: statement_sim(sy=39): 39 not in statbegsys, 39<>ident(0) -> NO-OP
...
{---------------------------------------- 247 program_procedure_array_replace_2}
...
{---------------------------------------- 248 program_procedure_array_replace_7}
(* PB-31: full programme->block->body->statement chain simulation
   mirrors pcom token flow for "program hello; begin writeln(...) end." *)
program pb31(output);
var
