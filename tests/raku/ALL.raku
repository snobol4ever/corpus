...
#------------------------------------------------------------ 136 test_try_basic
try { say "safe"; }
#--------------------- 137 ladder__rung07_subs_implicit_return_bare_literal_tail
sub seven() { 7; }
say seven();
#-------------------------------------------------------------------- 138 say_29
...
say("hello");
say(42);
#---------------------------------------------------------- 161 simple_assign_46
my constant GREETING = "hello";
say GREETING;
#---------------------------------------------------------- 162 simple_assign_47
my constant K = 3;
say K * K;
#---------------------------------------------------------- 163 simple_assign_48
constant $E = 2;
say $E;
#-------------------------------------------------------------------- 164 sub_26
sub t() { 42 }
...
sub greet() { "hi" }
say greet();
#----------------------------------------- 166 ladder__rung16_bool_relops_string
say ("a" eq "a");
say ("a" ne "a");
say ("a" lt "b");
#------------------------------------------------ 167 ladder__rung17_compare_ops
say (3 <=> 1);
say (1 <=> 3);
say (2 <=> 2);
#-------------------------------------------- 168 ladder__rung17_compare_ops_cmp
say (10 cmp 9);
say (1 cmp 2);
say ("b" cmp "b");
#-------------------------------------------- 169 ladder__rung17_compare_ops_leg
say ("abc" leg "abd");
say ("b" leg "a");
say ("x" leg "x");
#---------------------------------------- 170 ladder__rung17_compare_ops_numeric
say ((1 <=> 3) == -1);
say ((2 <=> 2) == 0);
say ((5 <=> 2) == 1);
#---------------------------------------------------------- 171 simple_assign_49
constant A = "a";
constant B = "b";
say A ~ B;
#---------------------------------------- 172 ladder__rung12_exceptions_no_catch
try {
    die "swallowed";
}
say "after";
#------------------------------------------------ 173 ladder__rung16_bool_relops
say (1 == 1);
say (1 < 2);
say (2 <= 1);
say (1 != 1);
...
say 2.5+1.5;
say -3.0;
#----------------------------------------- 175 ladder__rung11_junctions_operator
if 1|2|3 == 2 {
    say "yes";
} else {
    say "no";
}
#------------------------------------------------- 176 ladder__rung12_exceptions
try {
    die "boom";
    CATCH {
        default { say "caught"; }
...
#------------------------------------- 177 ladder__rung12_exceptions_die_message
try {
    die "custom message";
    CATCH {
        default { say .message; }
...
#--------------------------------------------- 201 ladder__rung19_block_methcall
say ("a","b").map({ $^a.uc });
#------------------------------------- 202 ladder__rung19_block_methcall_chained
say (1,2).map({ $^a.Str.chars });
#---------------------------------------- 203 ladder__rung19_block_methcall_grep
say ("a","b").grep({ $^a.uc eq "A" });
#------------------------------------------- 204 ladder__rung20_paren_group_grep
say (1,2,3).grep({ ($^a) > 1 });
#------------------------------------ 205 ladder__rung20_paren_group_placeholder
say (1,2).map({ ($^a).Str });
#----------------------------------------------------------------- 206 say_die_1
 say("before"); die("boom"); say("after"); 
#--------------------------------------------------------- 207 simple_assign_100
my $i = 7; say $i++;
#--------------------------------------------------------- 208 simple_assign_102
my $i = 5; my $j = --$i; say $j;
#--------------------------------------------------------- 209 simple_assign_105
my $i = 5; my $j = ++$i; say $j;
#--------------------------------------------------------- 210 simple_assign_106
my $i = 5; my $j = ++$i; say $i;
#--------------------------------------------------------- 211 simple_assign_110
my $x = 5; say "x is ", $x;
#--------------------------------------------------------- 212 simple_assign_111
my $x = 6; my $y = 7; say $x * $y;
#--------------------------------------------------------- 213 simple_assign_114
my $a = "zebra"; my $b = "apple"; say $a gt $b ?? "y" !! "n";
#--------------------------------------------------------- 214 simple_assign_115
my $x = 0 ?? 5 !! 9; say $x;
#--------------------------------------------------------- 215 simple_assign_116
my $g = "C"; say $g eq "A" ?? 1 !! $g eq "B" ?? 2 !! 3;
#--------------------------------------------------------- 216 simple_assign_117
my $n = 10; say $n > 3 ?? "big" !! "small";
#--------------------------------------------------------- 217 simple_assign_118
my $x = 1 ?? 5 !! 9; say $x;
#--------------------------------------------------------- 218 simple_assign_124
my $s; $s = "ab" x 3; say $s;
#--------------------------------------------------------- 219 simple_assign_125
my $s = "ab" x 3; say $s;
#--------------------------------------------------------- 220 simple_assign_126
my $n = 3; my $s = "ab" x $n; say $s;
#--------------------------------------------------------- 221 simple_assign_128
my $s = "yo"; my $n = 4; say $s x $n;
#---------------------------------------------------------- 222 simple_assign_26
 my $b = { say 9; }; $b(); $b(); 
#---------------------------------------------------------- 223 simple_assign_28
 my $b = { say 5; }; $b(); 
#---------------------------------------------------------- 224 simple_assign_29
 my $a = { say 1; }; my $b = { say 2; }; $b(); $a(); 
#---------------------------------------------------------- 225 simple_assign_67
my $x = 7; say $x.fmt("%03d");
#---------------------------------------------------------- 226 simple_assign_68
my $s = "hi"; say $s.fmt("[%5s]");
#---------------------------------------------------------- 227 simple_assign_69
my $i = 3; $i++; $i++; say $i;
#---------------------------------------------------------- 228 simple_assign_89
my $i = 5; say $i--;
#---------------------------------------------------------- 229 simple_assign_98
my $i = 5; my $j = $i++; say $j;
#---------------------------------------------------------- 230 simple_assign_99
my $i = 5; my $j = $i++; say $i;
#--------------------------------------------------------------- 231 split_say_1
 say("a,b,c".split(",")); 
#----------------------------------------------------------- 232 test_colon_call
my $x = "hi"; say $x.substr: 1;
#---------------------------------------------------------- 233 test_kebab_ident
my $s-address = 5; say $s-address;
#------------------------------------------------------------- 234 test_our_decl
our $a = 1; say $a;
#------------------------------------------------------- 235 test_qualified_term
...
my %h = a => 1, b => 2, c => 3;
say %h.kv.elems;
#--------------------------------- 266 ladder__rung01_variables_type_constrained
my Int $x = 5;
say $x;
#-------------------------------------- 267 ladder__rung04_arrays_negative_index
my @a = (1, 2, 3);
say @a[*-1];
#--------------------------------------- 268 ladder__rung04_arrays_out_of_bounds
my @a = (1, 2, 3);
say @a[10].raku;
#----------------------------------------- 269 ladder__rung05_hashes_missing_key
my %h = (a => 1);
say %h{'z'}.raku;
#-------------------------------------------- 270 ladder__rung06_control_ternary
my $x = 5;
say $x > 3 ?? "big" !! "small";
#---------------------------------------------- 271 ladder__rung06_control_until
my $i = 0;
until ($i >= 3) { say $i; $i = $i + 1; }
#--------------------------------------- 272 ladder__rung07_subs_implicit_return
sub double($x) { $x * 2 }
say double(5);
#-------------------- 273 ladder__rung07_subs_implicit_return_trailing_semicolon
sub double($x) { $x * 2; }
say double(5);
#----------------------------------------------------- 274 ladder__rung15_blocks
my $b = { 1 };
say $b();
#----------------------------------------------- 275 ladder__rung15_blocks_first
my @n = (1, 2, 3, 4);
say @n.first({ $^a == 4 });
#---------------------------------------- 276 ladder__rung15_blocks_placeholders
my $add = { $^a + $^b };
say $add(2, 3);
#---------------------------------------------- 277 ladder__rung15_blocks_pointy
my $p = -> $x { $x * 2 };
say $p(5);
#----------------------------------------------- 278 ladder__rung15_blocks_topic
my $b = { $_ * 2 };
say $b(5);
#----------------------------------- 279 ladder__rung18_two_arg_block_reduce_add
my @n = (1,2,3);
say @n.reduce({ $^a + $^b });
#----------------------------------- 280 ladder__rung18_two_arg_block_reduce_mul
my @n = (1,2,3,4);
say @n.reduce({ $^a * $^b });
#------------------------------------ 281 ladder__rung19_block_methcall_sub_body
sub f($x) { $x.uc }
say f("ab");
#------------------------------------------------ 282 ladder__rung20_paren_group
sub f($a) { ($a).Str }
say f(3);
#------------------------------------------- 283 ladder__rung20_paren_group_stmt
my $x = 5;
($x).say;
#------------------------------------------------------------------- 284 multi_3
multi bar($x) { $x + 1 }
...
#----------------------------------------------------------------- 285 say_try_1
    ...
    say('after');
#--------------------------------------------------------- 286 simple_assign_101
my $b = 5;
say $b ** 2;
#--------------------------------------------------------- 287 simple_assign_112
my $x = 10; my $y = 3;
say $x % $y;
#--------------------------------------------------------- 288 simple_assign_113
my $x = 10; my $y = 3;
say $x * $y;
#--------------------------------------------------------- 289 simple_assign_123
my $d = 42;
{ say "def" with $d }
#---------------------------------------------------------- 290 simple_assign_38
my $n = 66;
say chr($n);
#---------------------------------------------------------- 291 simple_assign_51
my $x = 0;
say $x // "oops";
#---------------------------------------------------------- 292 simple_assign_54
my ($a, $b) = (10, 20);
say $a + $b;
#---------------------------------------------------------- 293 simple_assign_55
my ($x, $y) = "foo", "bar";
say $y;
#---------------------------------------------------------- 294 simple_assign_56
my ($a, $b, $c) = 1, 2, 3;
say $c;
#---------------------------------------------------------- 295 simple_assign_73
my $s = "hello";
say $s.chars;
#---------------------------------------------------------- 296 simple_assign_74
my $x = 5;
say "yes" with $x;
#---------------------------------------------------------- 297 simple_assign_76
my $x = 42;
say $_ with $x;
#---------------------------------------------------------- 298 simple_assign_77
my $x = 0;
say "z" with $x;
#---------------------------------------------------------- 299 simple_assign_85
my $i = 0;
until $i >= 3 { say $i; $i = $i + 1 }
#---------------------------------------------------------- 300 simple_assign_87
my $c = "z";
say $c.ord;
#--------------------------------------------------------- 301 simple_program_26
my $y;
say "gone" without $y;
#---------------------------------------------------------- 302 simple_program_9
my $x;
say $x // "default";
#-------------------------------------------------------------------- 303 sub_12
...
sub f(*%h) { say %h{'z'}; }
f(z => 4, w => 5);
#--------------------------------------------------- 319 test_interp_leading_var
my $x = "world";
say "$x there";
#------------------------------------------------------ 320 test_interp_only_var
my $x = "hi";
say "$x";
#-------------------------------------------------------- 321 test_interp_simple
my $name = "world";
say "hello $name";
#-------------------------------------------------- 322 test_interp_trailing_lit
my $a = "foo";
say "val=$a!";
#---------------------------------------------------------- 323 test_print_basic
my $x = "hello";
print "$x\n";
#------------------------------------------------------------------ 324 array_14
my @n = (1,2,3,4);
...
say @a[*-1];
say @a[*-2];
#----------------------------- 329 ladder__rung01_variables_multiple_declaration
my ($a, $b) = (1, 2);
say $a;
say $b;
#----------------------------------- 330 ladder__rung01_variables_no_initializer
my $x;
$x = 5;
say $x;
#----------------------------------------------------- 331 ladder__rung05_hashes
my %h = a => 1, b => 2;
say %h{'a'};
say %h{'b'};
#----------------------------------------- 332 ladder__rung05_hashes_keys_values
my %h = (a => 1, b => 2);
say %h.keys.sort;
say %h.values.sort;
#--------------------------------------------- 333 ladder__rung09_string_methods
my $s = "Raku";
say $s.uc;
say $s.chars;
#------------------------------------- 334 ladder__rung10_given_when_basic_match
given 5 {
    when 5 { say "matched"; }
}
#----------------------------------- 335 ladder__rung16_bool_relops_interpolated
my $b = 3 > 1;
my $c = 3 < 1;
say "b=$b c=$c";
#--------------------------------------------------------- 336 simple_assign_103
my $n = 5;
--$n;
say $n;
#--------------------------------------------------------- 337 simple_assign_104
my $n = 5;
++$n;
say $n;
#---------------------------------------------------------- 338 simple_assign_25
my $s = 1;
$s = 2;
say $s;
#---------------------------------------------------------- 339 simple_assign_39
my $x = 5;
$x += 3;
say $x;
#---------------------------------------------------------- 340 simple_assign_41
my $x = 24;
$x /= 6;
say $x;
#---------------------------------------------------------- 341 simple_assign_42
my $x = 6;
$x *= 4;
say $x;
#---------------------------------------------------------- 342 simple_assign_43
my $x = 7;
$x -= 4;
say $x;
#---------------------------------------------------------- 343 simple_assign_52
my ($a, $b) = 1, 2, 3;
say $a;
say $b;
#---------------------------------------------------------- 344 simple_assign_53
my ($a, $b, $c) = 1, 2;
say $a;
say $b;
#---------------------------------------------------------- 345 simple_assign_57
my ($a, $b) = 3, 7;
say $a;
say $b;
#---------------------------------------------------------- 346 simple_assign_58
my $h = 7/2;
say $h*2 == 7 ?? 7 !! 0;
say $h*2;
#---------------------------------------------------------- 347 simple_assign_78
my $x = 3;
say "nope" without $x;
say "end";
#---------------------------------------------------------- 348 simple_assign_97
my $i = 0;
$i = $i + 1 until $i >= 3;
say $i;
#--------------------------------------------------------- 349 simple_program_25
my $y;
say "nope" with $y;
say "end";
#----------------------------------------------------- 350 test_interp_multi_var
my $a = "foo";
my $b = "bar";
...
#------------------------------------------------------------ 353 junction_say_6
    ...
    say(none(10, 20));
    say(one('x', 'y'));
#-------------------------------------------------- 354 ladder__rung01_variables
my $name = "Raku";
my $n = 42;
say $name;
say $n;
#----------------------------------------------------- 355 ladder__rung04_arrays
my @arr = (1, 2, 3);
say @arr[0];
say @arr[2];
say @arr.elems;
#--------------------------------------- 356 ladder__rung05_hashes_exists_delete
my %h = (a => 1);
say %h{'a'}:exists;
%h{'a'}:delete;
say %h{'a'}:exists;
#------------------------------------ 357 ladder__rung10_given_when_default_case
given 99 {
    when 1 { say "one"; }
    default { say "other"; }
}
#-------------------------- 358 ladder__rung10_given_when_no_match_falls_through
given 99 {
    when 1 { say "one"; }
}
say "after";
#--------------------------- 359 ladder__rung17_compare_ops_in_a_boolean_context
say ((1 <=> 2) ?? "T" !! "F");
say ((2 <=> 2) ?? "T" !! "F");
say (("a" cmp "b") ?? "T" !! "F");
if (3 <=> 3) { say "y" } else { say "n" }
#--------------------------------------- 360 ladder__rung17_compare_ops_variable
my $o = (5 <=> 2);
say $o;
my $p = ("a" leg "b");
say $p;
#--------------------------------------------------------- 361 simple_assign_109
my $n = 7;
repeat {
    say $n;
} until $n > 0;
#---------------------------------------------------------- 362 simple_assign_50
my $a = "ok";
my $b;
say $a // "nope";
say $b // "fallback";
#---------------------------------------------------------- 363 simple_assign_59
my $a = 5; my $b = 2;
say $a/$b;
my $c = 8; my $d = 2;
say $c/$d;
#---------------------------------------------------------- 364 simple_assign_75
my $x = 7;
my $r = 0;
$r = $_ with $x;
say $r;
#---------------------------------------------------------- 365 simple_assign_79
my $y;
my $r = 0;
$r = 9 without $y;
say $r;
#---------------------------------------------------------- 366 simple_assign_86
my ($a, $b, $c) = 3, 7;
say $a;
say $b;
say "end";
#-------------------------------------------- 367 ladder__rung04_arrays_push_pop
my @a = (1, 2);
@a.push(3);
say @a;
@a.pop;
say @a;
#----------------------------------- 368 ladder__rung10_given_when_multiple_when
given 2 {
    when 1 { say "one"; }
    when 2 { say "two"; }
    when 3 { say "three"; }
}
#-------------------------------------------------- 369 ladder__rung11_junctions
if any(1,2,3) == 2 {
    say "yes";
} else {
    say "no";
}
#---------------------------------------------- 370 ladder__rung11_junctions_all
if all(1,2,3) > 0 {
    say "yes";
} else {
    say "no";
}
#--------------------------------------------- 371 ladder__rung11_junctions_none
if none(1,2,3) == 5 {
    say "yes";
} else {
    say "no";
}
#---------------------------------------------- 372 ladder__rung11_junctions_one
if one(1,2,3) == 2 {
    say "yes";
} else {
    say "no";
}
#--------------------------------------------------------- 373 simple_assign_108
my $j = 10;
repeat {
    $j = $j + 1;
    say $j;
} until $j >= 12;
#---------------------------------------------------------- 374 simple_assign_90
my $i = 3;
$i--;
say $i;
$i--;
say $i;
#---------------------------------------------------------- 375 simple_assign_94
my $i = 0;
$i++;
say $i;
$i++;
...
#--------------------------------------- 393 ladder__rung19_block_methcall_topic
say ("foo","bar").map({ $_.substr(1,1) });
#---------------------------------------- 394 ladder__rung20_paren_group_chained
say (1,2).map({ ($^a).Str }).join(",");
#------------------------------------------ 395 ladder__rung20_paren_group_topic
say (1,2).map({ ($_).Str });
#-------------------------------------------------------------------- 396 loop_1
...
#----------------------------------------------- 434 ladder__rung04_arrays_slice
my @a = (1, 2, 3, 4, 5);
say @a[1..3];
#--------------------------------------------- 435 ladder__rung06_control_unless
my $x = 5;
unless ($x > 10) { say "small"; }
#-------------------------------------------- 436 ladder__rung15_blocks_anon_sub
my $f = sub { 1 };
say $f();
#----------------------------------- 437 ladder__rung15_blocks_anon_sub_with_arg
my $f = sub ($arg) { 1 + $arg };
say $f(3);
#---------------------------------------------- 438 ladder__rung18_two_arg_block
my @n = (3,1,2);
say @n.sort({ $^b <=> $^a });
#-------------------------------- 439 ladder__rung18_two_arg_block_reduce_concat
my @w = ("a","bb","c");
say @w.reduce({ $^a ~ $^b });
#---------------------------------- 440 ladder__rung18_two_arg_block_sort_string
my @w = ("pear","fig","kiwi");
say @w.sort({ $^b leg $^a });
#--------------------------------- 441 ladder__rung19_block_methcall_sort_by_key
my @w = <aa b ccc>;
...
#------------------------------------------------------------- 459 say_try_die_4
    ...
    say('after');
#--------------------------------------------------------- 460 simple_assign_120
my $x = 5;
{ say "big" if $x > 3 }
#--------------------------------------------------------- 461 simple_assign_121
my $x = 1;
{ say "hi" unless $x }
#--------------------------------------------------------- 462 simple_assign_122
my $x = 0;
{ say "hi" unless $x }
#---------------------------------------------------------- 463 simple_assign_30
my $x = 5;
if ($x > 3) { say "big" }
#---------------------------------------------------------- 464 simple_assign_31
my $x = 5;
if 10 > $x > 1 { say "gt" }
#---------------------------------------------------------- 465 simple_assign_32
my $x = 5;
if 1 < $x < 10 { say "in" }
#---------------------------------------------------------- 466 simple_assign_33
my $x = 1;
if 1 <= $x < 10 { say "in" }
#---------------------------------------------------------- 467 simple_assign_34
my $a = 5;
if 1 < $a == 5 { say "m" }
#---------------------------------------------------------- 468 simple_assign_35
my $x = 50;
if 1 < $x < 10 { say "bad" } else { say "out" }
#---------------------------------------------------------- 469 simple_assign_36
my $x = 1;
if 1 < $x < 10 { say "bad" } else { say "out" }
#---------------------------------------------------------- 470 simple_assign_37
my $x = 5;
unless 1 < $x < 3 { say "fires" }
#---------------------------------------------------------- 471 simple_assign_60
my $n = 8;
if $n %% 2 { say "even" } else { say "odd" }
#---------------------------------------------------------- 472 simple_assign_61
my $n = 7;
if $n %% 2 { say "even" } else { say "odd" }
#---------------------------------------------------------- 473 simple_assign_72
my $i = 0;
until $i >= 10 { last if $i >= 2; say $i; $i = $i + 1 }
#---------------------------------------------------------- 474 simple_assign_81
my $x = 2;
if $x == 1 { say "one" } else { if $x == 2 { say "two" } else { say "other" } }
#---------------------------------------------------------- 475 simple_assign_82
my $x = 5;
if $x > 3 { say "big" }
#---------------------------------------------------------- 476 simple_assign_83
my $x = 2;
if $x > 3 { say "big" } else { say "small" }
#---------------------------------------------------------- 477 simple_assign_84
my $x = 2;
unless $x > 5 { say "not big" }
#---------------------------------------------------------- 478 simple_assign_88
my $x = 5;
if ($x > 3) { say "big"; }
#---------------------------------------------------------- 479 simple_assign_91
my $x = 1;
say "big" if $x > 3;
#---------------------------------------------------------- 480 simple_assign_93
my $x = 5;
say "big" if $x > 3;
#---------------------------------------------------------- 481 simple_assign_95
my $ok = 1;
print "hi" if $ok == 1;
#---------------------------------------------------------- 482 simple_assign_96
my $x = 2;
...
say(@lines);
#------------------------------------------------- 501 test_stmt_ctrl_loop_three
my $i = 0;
loop ($i = 0; $i < 3; $i = $i + 1) { say $i; }
#---------------------------------------------------------- 502 test_stmt_pfx_do
my $x = 1;
do { say($x); };
#------------------------------------------------------------------- 503 while_4
my $i = 0;
...
@a = 5 xx 3;
say @a.join(",");
#-------------------------------- 513 ladder__rung16_bool_relops_numeric_context
say (3 > 1) + 1;
my $b = 5 < 9;
say $b + 0;
#------------------------------------------- 514 ladder__rung20_paren_group_list
my $x = 5;
my $y = 6;
($x, $y).join("-").say;
#------------------------------------------------------------------- 515 multi_1
...
#------------------------------------------------------------- 518 say_try_die_3
    ...
    say('unreached');
#--------------------------------------------------------- 519 simple_assign_107
my $x = 4; my $y = 9;
if ($x < $y) { say "ct" } else { say "cf" }
if (4 < 9) { say "lt" } else { say "lf" }
#--------------------------------------------------------- 520 simple_assign_119
my $x = 10;
{ $x = $x + 1 if $x > 5 }
say $x;
#---------------------------------------------------------- 521 simple_assign_40
my $s = "hello";
$s ~= " world";
say $s;
#---------------------------------------------------------- 522 simple_assign_80
my $a = 1;
my $b = 0;
...
my @a = (1,2,3);
f(@a);
#------------------------------------------------------- 524 test_stmt_pfx_ENTER
my $x = 1;
ENTER { say("enter"); }
say($x);
#-------------------------------------------------------- 525 test_stmt_pfx_KEEP
my $x = 1;
KEEP { say("keep"); }
say($x);
#------------------------------------------------------- 526 test_stmt_pfx_LEAVE
my $x = 1;
LEAVE { say("leave"); }
say($x);
#-------------------------------------------------------- 527 test_stmt_pfx_POST
my $x = 5;
POST { say("post"); }
say($x);
#--------------------------------------------------------- 528 test_stmt_pfx_PRE
my $x = 5;
PRE { say("pre"); }
say($x);
#-------------------------------------------------------- 529 test_stmt_pfx_TEMP
my $x = 1;
TEMP { say("temp"); }
say($x);
#-------------------------------------------------------- 530 test_stmt_pfx_UNDO
my $x = 1;
UNDO { say("undo"); }
say($x);
#------------------------------------------------------------------- 531 while_5
...
#--------------------------------------------------------------- 535 array_say_5
    ...
    say(@r[1]);
    say(@r[2]);
#--------------------------------- 536 ladder__rung10_given_when_when_with_range
given 5 {
    when 1..3 { say "low"; }
    when 4..6 { say "mid"; }
    default { say "high"; }
}
#-------------------------------------------- 537 ladder__rung12_exceptions_fail
sub f {
    fail "oops";
}
my $r = f();
...
#---------------------------------------------- 539 benchmark_rc-man-or-boy-test
...
say A(10, {1}, {-1}, {-1}, {1}, {0});
#----------------------------------- 540 ladder__rung12_exceptions_specific_type
try {
...
#-------------------------------------------------------- 573 join_range_array_2
my @r = 1..5;
say @r[0,2,4].join(",");
#-------------------------------------------------------- 574 join_range_array_3
my @a = 10,20,30,40,50;
say @a[1..2].join(",");
#-------------------------------------------------------- 575 join_range_array_4
my @a = 1..4;
say @a.join(",");
#-------------------------------------------------------- 576 join_range_array_5
my @r = 10..20;
say @r[2..5].join(",");
#-------------------------------------------------------- 577 join_range_array_6
my @a = 1,2,3,4,5;
say @a[1..^4].join(",");
#-------------------------------------------------------- 578 join_range_array_7
my @a = 1,2,3,4,5;
say @a[0..2].join(",");
#-------------------------------------------------------- 579 join_range_array_8
my @a = 1,2,3,4,5;
say @a[1..3].join(",");
#-------------------------------------------------------- 580 join_range_array_9
my @a = 1,2,3,4,5;
say @a[2..2].join(",");
#---------------------------------------- 581 ladder__rung06_control_elsif_chain
my $x = 2;
if ($x == 1) { say "one"; } elsif ($x == 2) { say "two"; } else { say "other"; }
#------------------------------------------------------- 582 ladder__rung07_subs
sub add($a, $b) { return $a + $b; }
say add(3, 4);
#------------------------------------------------ 583 ladder__rung15_blocks_grep
my @n = (1, 2, 3, 4);
say @n.grep({ $_ > 2 });
#------------------------------------------------- 584 ladder__rung15_blocks_map
my @n = (1, 2, 3, 4);
say @n.map({ $_ * 2 });
#-------------------------------------------------------------------- 585 loop_2
my $i = 0;
...
#--------------------------------------------------- 635 class_method_replace_10
class C { method sq($n) { $n * $n } }
my $o = C.new;
say $o.sq(6);
#--------------------------------------------------- 636 class_method_replace_11
class C { method calc($n) { my $t = $n + 1; $t * 2 } }
my $o = C.new;
say $o.calc(4);
#--------------------------------------------------- 637 class_method_replace_14
class C { method answer { 42 } }
my $o = C.new;
say $o.answer();
#---------------------------------------------------- 638 class_method_replace_6
class C { method go() { 7 } }
my $x = C.new.go();
say $x;
#----------------------------------------------------------- 639 class_replace_1
...
my $i = 1;
say @a[$i,3].join(",");
#-------------------------------------------------------- 644 join_range_array_1
my @a = 9;
@a = 1..4;
say @a.join(",");
#--------------------------------------- 645 ladder__rung07_subs_optional_params
...
#----------------------------------- 664 ladder__rung13_classes_public_attribute
class Point {
    has $.x is rw;
    has $.y is rw;
}
...
$p.x = 10;
say $p.x;
#----------------------------------------- 665 benchmark_pi-sequential-iteration
...
#------------------------------------------ 693 ladder__rung07_subs_named_params
sub greet(:$name) { return "hi " ~ $name; }
say greet(name => "Bob");
#------------------------------------ 694 ladder__rung08_for_loop_for_over_array
my @a = (10, 20, 30);
for @a -> $x { say $x; }
#------------------------------------ 695 ladder__rung08_for_loop_for_with_index
my @a = ("a", "b");
for @a.kv -> $i, $v { say "$i:$v"; }
#------------------------------------------------------ 696 method_say_replace_1
role Speaks { method talk() { return "hello"; } }
 my $s = Speaks.new(); say($s.talk()); 
#--------------------------------------------------------- 697 multi_sub_array_3
multi sub f($a, **@r) { say "L:" ~ @r.elems; }
f(1,2,3,4);
#--------------------------------------------------------- 698 multi_sub_array_4
...
#------------------------------------------------------ 714 class_say_replace_26
class Animal { has $.name; }
class Dog is Animal { }
 my $d = Dog.new(name => "Rex"); say($d.name); 
#------------------------------------------------------ 715 class_say_replace_27
role HasId { has $.id; }
...
#--------------------------------------------------- 727 class_method_replace_16
class C { method m($v = 42) { return $v } }
my $o = C.new;
say $o.m();
say $o.m(8);
#----------------------------------------------- 728 class_method_say_replace_35
class A { method TWEAK() { say("A"); } }
class B is A { method TWEAK() { say("B"); } }
class C is B { method TWEAK() { say("C"); } }
 my $c = C.new(); 
#------------------------------------------------------- 729 join_range_array_10
my @a = 1,2,3,4,5;
my $lo = 1;
my $hi = 3;
say @a[$lo..$hi].join(",");
#---------------------------------------- 730 ladder__rung07_subs_multi_dispatch
...
sub boomer() { die('deep'); say('sub-after'); };
    ...
    say('after');
#------------------------------------------------------- 732 test_stmt_pfx_FIRST
for 1..3 -> $i {
    FIRST { say("first"); }
    say($i);
}
#-------------------------------------------------------- 733 test_stmt_pfx_LAST
for 1..3 -> $i {
    LAST { say("last"); }
    say($i);
}
#-------------------------------------------------------- 734 test_stmt_pfx_NEXT
for 1..3 -> $i {
    NEXT { say("next"); }
    say($i);
}
#------------------------------------------------------------- 735 say_try_die_5
    ...
    say('after');
#------------------------------------------------------ 736 test_nested_if_while
my $i = 0;
if ($i == 0) {
    while ($i < 3) { $i = $i + 1; }
}
say($i);
#------------------------------------------------------- 737 token_say_replace_1
grammar G {
    token TOP { \d+ }
}
...
my $g = Greeter.new;
say $g.greet;
#------------------------------ 739 ladder__rung13_classes_named_arg_constructor
class Sum {
    has $.a;
    has $.b;
    method total { $!a + $!b }
...
say $s.total;
#------------------------------------------- 740 ladder__rung14_roles_smartmatch
role Flyer { }
...
my $w = Widget.new;
say $w.greet;
#--------------------------------------------- 743 ladder__rung14_roles_override
role Greeter {
...
my $s = Special.new;
say $s.greet;
#------------------------------------------- 744 ladder__rung14_roles_attributes
role Counter {
    has $.count is rw = 0;
    method inc { $!count++ }
}
...
$t.inc;
say $t.count;
#------------------------------------------ 745 ladder__rung14_roles_composition
role Flyer {
...
say $d.fly;
say $d.swim;
#---------------------------------- 746 ladder__rung13_classes_private_attribute
class Counter {
    has $!count;
    method init { $!count = 0 }
    method inc { $!count++ }
...
#----------------------------------------- 922 class_method_say_replace_41 XFAIL
class Base { has $!secret; method peek() { return $!secret; } }
class Derived is Base { }
 my $d = Derived.new(secret => 5); say($d.peek()); 
#----------------------------------------------------- 923 multi_sub_say_3 XFAIL
