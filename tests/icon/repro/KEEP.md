...
| file | status |
|---|---|
| `table_key_order.icn` (+ `.ref`) | Oracle-confirmed wrong-answer bug: SCRIP disagrees with both this file's own `.ref` AND real Arizona `icont` (which agree with each other) on table iteration order, traced to `BID_key` sharing the s262 SNOBOL4 `TABLE` rewrite's bucket structure — a live SHARED-NODE VERDICT SCOPE gap (RULES.md:185), not a crash. Full trace: `.github/FINDING-2026-08-28-seat08-icon-table-key-order-diverges-from-arizona-since-the-snobol4-table-rewrite.md`. Mailed `hq_C` (`icon-table-key-order-diverges-from-arizona`), unresolved as of this session — re-check before assuming still open. |
