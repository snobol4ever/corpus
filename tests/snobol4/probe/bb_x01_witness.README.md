...
⭐ **RELOCATED 2026-08-28** (probe-consolidate-bb, LON-20260828 total conversion): the seven files this
README describes converted from loose `corpus/probe/bb/probes_x01_witness_s39/*.sno`+`.ref` into
`bb_x01_witness.sno`/`.ref` beside this file (harness `corpus_suite_harness.py`, byte-equal-or-no-delete
proven both directions, both modes). ⭐ **ALSO NOW GREEN**: the table below is the s39 snapshot and reads
`SCRIP got: =F` for six of seven rows — re-verified live during this conversion (2026-08-28), all seven
entries now PASS (`=S`, matching the oracle) under a pristine build. The bug this witness set was built to
pin down has since been fixed; kept for its historical analysis, not as an open-defect tracker.
