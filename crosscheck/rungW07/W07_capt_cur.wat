...
(module
  ...
  (data (i32.const 65536) "ABCDEABCPASS W07/001: cursor pos=FAIL W07/001: cursor capture should matchPASS W07/002: cursor after match=FAIL W07/002: cursor after match should match")
  ...
  (global $var_pos2_off (mut i32) (i32.const 0))
  (global $var_pos2_len (mut i32) (i32.const 0))
  ...
  (func (export "main") (result i32)
    ...
    (global.get $var_subject_len)
      ...
      (call $sno_pat_search)
      (local.set $pat_cursor)
      ;; E_CAPT_CUR @pos2: store cursor position as int string into var
      (if (i32.ge_s (local.get $pat_cursor) (i32.const 0)) (then
        (i64.extend_i32_u (local.get $pat_cursor))
        (call $sno_int_to_str)
        (global.set $var_pos2_len)
        (global.set $var_pos2_off)
      ))
      (local.set $ok (i32.ge_s (local.get $pat_cursor) (i32.const 0)))
      (if (i32.eqz (local.get $ok)) (then
    ...
    (i32.const 65610)
    (i32.const 33)
    (global.get $var_pos2_off)
    (global.get $var_pos2_len)
    (call $sno_str_concat)
      (call $sno_output_str)
      (local.set $ok (i32.const 1))
