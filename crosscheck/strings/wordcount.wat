...
(module
  ...
  (data (i32.const 65536) "0123456789'-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz words")

  ;; SNOBOL4 variables: each → (offset, length) global pair
  (global $var_NUMERALS_off (mut i32) (i32.const 0))
  (global $var_NUMERALS_len (mut i32) (i32.const 0))
  (global $var_WORD_off (mut i32) (i32.const 0))
  (global $var_WORD_len (mut i32) (i32.const 0))
  (global $var_WPAT_off (mut i32) (i32.const 0))
  (global $var_WPAT_len (mut i32) (i32.const 0))
  ...
  (func (export "main") (result i32)
    ...
    (i32.const 65536)
    (i32.const 10)
      (global.set $var_NUMERALS_len)
      (global.set $var_NUMERALS_off)
      (local.set $ok (i32.const 1))
      ;; WORD = ...
    (i32.const 65546)
    (i32.const 2)
    (global.get $var_NUMERALS_off)
    (global.get $var_NUMERALS_len)
    (call $sno_str_concat)
    (i32.const 65548) ;; &UCASE
    (i32.const 26)
    (call $sno_str_concat)
    (i32.const 65574) ;; &LCASE
    (i32.const 26)
    (call $sno_str_concat)
      (global.set $var_WORD_len)
      (global.set $var_WORD_off)
      (local.set $ok (i32.const 1))
      ;; WPAT = ...
    (global.get $var_WORD_off)
    (global.get $var_WORD_len)
    (drop)
    (drop)
    ;; unimplemented builtin 'BREAK' in value ctx — empty string
    (i32.const 65536)
    (i32.const 0)
    (global.get $var_WORD_off)
    (global.get $var_WORD_len)
    ...
    (i32.const 0)
    (call $sno_str_concat)
      (global.set $var_WPAT_len)
      (global.set $var_WPAT_off)
      (local.set $ok (i32.const 1))
      ) ;; $L1 NEXTL
      ;; LINE = ...
    ...
    (i32.const 0)
      ...
      (local.set $ok (i32.const 1))
      (if (i32.eqz (local.get $ok)) (then
      (local.set $pc (i32.const 1)) ;; → NEXTL
      (br $dispatch)
      ))
      ;; N = ...
    ...
    (i64.add)
      ...
      (local.set $ok (i32.const 1))
      (local.set $pc (i32.const 2)) ;; → NEXTW
      (br $dispatch)
      ) ;; $L3 DONE
      ;; OUTPUT = ...
    ;; UNHANDLED EKind 10
