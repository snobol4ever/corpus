...
(module
  ...
  (data (i32.const 65536) "abcFAIL 913/001: integer(string) should failFAIL 913/002: integer(12) should succeed12FAIL 913/003: integer(numeric-string) should succeedPASS 913_integer_pred (3/3)")
  ...
  (func (export "main") (result i32)
    ...
    (i32.const 65536)
    (i32.const 3)
      ;; INTEGER predicate: scan string for all-digit
      (local.set $int_pred_len)
      (local.set $int_pred_off)
      (local.set $int_pred_i (i32.const 0))
      (local.set $int_pred_ok (i32.const 1))
      (if (i32.gt_u (local.get $int_pred_len) (i32.const 0))
        (then
          (local.set $int_pred_c (i32.load8_u (local.get $int_pred_off)))
          (if (i32.or (i32.eq (local.get $int_pred_c) (i32.const 43))
                      (i32.eq (local.get $int_pred_c) (i32.const 45)))
            (then (local.set $int_pred_i (i32.const 1))))))
      (if (i32.ge_u (local.get $int_pred_i) (local.get $int_pred_len))
        (then (local.set $int_pred_ok (i32.const 0))))
      (block $ipb (loop $ipl
        (br_if $ipb (i32.ge_u (local.get $int_pred_i) (local.get $int_pred_len)))
        (local.set $int_pred_c (i32.load8_u
          (i32.add (local.get $int_pred_off) (local.get $int_pred_i))))
        (if (i32.or
              (i32.lt_u (local.get $int_pred_c) (i32.const 48))
              (i32.gt_u (local.get $int_pred_c) (i32.const 57)))
          (then (local.set $int_pred_ok (i32.const 0)) (br $ipb)))
        (local.set $int_pred_i (i32.add (local.get $int_pred_i) (i32.const 1)))
        (br $ipl)))
      (local.get $int_pred_ok)
      (local.set $ok)
    ...
    (i32.const 41)
      ...
      ;; subject eval
    (i64.const 12)
      (drop) ;; integer literal — always succeed
      (i32.const 1)
      (local.set $ok)
