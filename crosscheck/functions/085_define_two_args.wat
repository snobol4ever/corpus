...
(module
  ...
  (data (i32.const 65536) "add(a,b)")

  ;; SNOBOL4 variables: each → (offset, length) global pair
  (global $var_add_off (mut i32) (i32.const 0))
  (global $var_add_len (mut i32) (i32.const 0))
  ...
  (func (export "main") (result i32)
    ...
    (i32.const 8)
      ...
      (local.set $ok)
      (local.set $pc (i32.const 2)) ;; → add_end
      (br $dispatch)
      ) ;; $L1 add
      ;; add = ...
    ...
    (i64.add)
      (call $sno_int_to_str)
      (global.set $var_add_len)
      (global.set $var_add_off)
    ...
    (i64.const 4)
    (drop)
    ;; unimplemented builtin 'add' in value ctx — empty string
    (i32.const 65536)
    (i32.const 0)
      (call $sno_output_str)
      (local.set $ok (i32.const 1))
