...
(module
  ...
  (data (i32.const 65536) "upcase(s)abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZhelloworld")

  ;; SNOBOL4 variables: each → (offset, length) global pair
  (global $var_upcase_off (mut i32) (i32.const 0))
  (global $var_upcase_len (mut i32) (i32.const 0))
  ...
  (func (export "main") (result i32)
    ...
    (i32.const 9)
      ...
      (local.set $ok)
      (local.set $pc (i32.const 2)) ;; → upcase_end
      (br $dispatch)
      ) ;; $L1 upcase
      ;; upcase = ...
    (global.get $var_s_off)
    (global.get $var_s_len)
    (i32.const 65545) ;; &LCASE
    (i32.const 26)
    (i32.const 65571) ;; &UCASE
    (i32.const 26)
    (call $sno_replace)
      (global.set $var_upcase_len)
      (global.set $var_upcase_off)
    ...
    (drop)
    (drop)
    ;; unimplemented builtin 'upcase' in value ctx — empty string
    (i32.const 65536)
    (i32.const 0)
      (call $sno_output_str)
      (local.set $ok (i32.const 1))
