...
(module
  ...
  (data (i32.const 65536) "swap(a,b)tmp helloworld")

  ;; SNOBOL4 variables: each → (offset, length) global pair
  (global $var_tmp_off (mut i32) (i32.const 0))
  (global $var_tmp_len (mut i32) (i32.const 0))
  ...
  (func (export "main") (result i32)
    ...
    (i32.const 12)
      ...
      (local.set $ok)
      (local.set $pc (i32.const 2)) ;; → swap_end
      (br $dispatch)
      ) ;; $L1 swap
      ;; tmp = ...
    (global.get $var_a_off)
    (global.get $var_a_len)
      (global.set $var_tmp_len)
      (global.set $var_tmp_off)
    ...
    (global.get $var_b_len)
      ...
      (local.set $ok (i32.const 1))
      ;; b = ...
    (global.get $var_tmp_off)
    (global.get $var_tmp_len)
    ...
    (i32.const 5)
      (drop)
      (drop)
      (i32.const 1) ;; builtin 'swap' — treated as succeed
      (local.set $ok)
      ) ;; $L3 END
      ;; END
