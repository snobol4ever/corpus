...
(module
  ...
  (data (i32.const 65536) "always_fail()FAIL 1014/001: freturn should cause statement failurePASS 1014_func_freturn (1/1)")
  ...
  (func (export "main") (result i32)
    ...
    (i32.const 13)
      ...
      (local.set $ok)
      (local.set $pc (i32.const 2)) ;; → af_end
      (br $dispatch)
      ) ;; $L1 always_fail
      (local.set $ok (i32.const 1)) ;; no subject
      (local.set $pc (i32.const 5)) ;; → freturn
      (br $dispatch)
      ) ;; $L2 af_end
      (local.set $ok (i32.const 1)) ;; no subject
      ;; subject eval
      (i32.const 1) ;; builtin 'always_fail' — treated as succeed
      (local.set $ok)
