...
(module
  ...
  (data (i32.const 65536) "jlab(a,b,c)d,e,fAFAIL 1017/001: arg(.jlab,1) = ACFAIL 1017/002: arg(.jlab,3) = CFAIL 1017/003: arg(.jlab,0) OOB should failFAIL 1017/004: arg(.jlab,4) OOB should failDFAIL 1017/005: local(.jlab,1) = DFFAIL 1017/006: local(.jlab,3) = FFAIL 1017/007: local(.jlab,0) OOB should failFAIL 1017/008: local(.jlab,4) OOB should failPASS 1017_arg_local (8/8)")

  ;; SNOBOL4 variables: each → (offset, length) global pair
  (global $var_jlab_off (mut i32) (i32.const 0))
  (global $var_jlab_len (mut i32) (i32.const 0))
  ...
  (func (export "main") (result i32)
    ...
    (i64.const 1)
    (drop)
    ;; unimplemented builtin 'arg' in value ctx — empty string
    (i32.const 65536)
    (i32.const 0)
    (i32.const 65552)
    (i32.const 1)
    ...
    (i64.const 0)
      (drop)
      (i32.const 1) ;; builtin 'arg' — treated as succeed
      (local.set $ok)
      (if (i32.eqz (local.get $ok)) (then
    ...
    (i64.const 1)
    (drop)
    ;; unimplemented builtin 'local' in value ctx — empty string
    (i32.const 65536)
    (i32.const 0)
    (i32.const 65702)
    (i32.const 1)
    ...
    (i64.const 0)
      (drop)
      (i32.const 1) ;; builtin 'local' — treated as succeed
      (local.set $ok)
      (if (i32.eqz (local.get $ok)) (then
      (local.set $pc (i32.const 8)) ;; → e007
      (br $dispatch)
      ))
      ;; OUTPUT = ...
    (i32.const 65770)
