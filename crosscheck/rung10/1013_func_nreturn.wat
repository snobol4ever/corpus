...
(module
  ...
  (data (i32.const 65536) "ref_a()FAIL 1013/001: nreturn read gives valueFAIL 1013/002: nreturn lvalue assign failedFAIL 1013/003: a updated via nreturnPASS 1013_func_nreturn (3/3)")

  ;; SNOBOL4 variables: each → (offset, length) global pair
  (global $var_ref_a_off (mut i32) (i32.const 0))
  (global $var_ref_a_len (mut i32) (i32.const 0))
  ...
  (func (export "main") (result i32)
    ...
    (i32.const 7)
      ...
      (local.set $ok)
      (local.set $pc (i32.const 2)) ;; → ref_a_end
      (br $dispatch)
      ) ;; $L1 ref_a
      ;; ref_a = ...
    ;; UNHANDLED EKind 40
    (i32.const 65536)
    (i32.const 0)
      (global.set $var_ref_a_len)
      (global.set $var_ref_a_off)
      (local.set $ok (i32.const 1))
      (local.set $pc (i32.const 7)) ;; → nreturn
      (br $dispatch)
      ) ;; $L2 ref_a_end
    ...
    (i64.const 27)
      ...
      (local.set $ok (i32.const 1))
      ;; subject eval
    ;; unimplemented builtin 'ref_a' in value ctx — empty string
    (i32.const 65536)
    (i32.const 0)
    (i64.const 27)
      (call $sno_int_to_str)
    ...
    (i32.const 39)
      ...
      (local.set $ok (i32.const 1)) ;; no subject
      ;; subject eval
      (i32.const 1) ;; builtin 'ref_a' — treated as succeed
      (local.set $ok)
