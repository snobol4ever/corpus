# seed/test_sno_cell_4.s — ζ-CELL MACHINE reference embodiment #4   (Lon + Claude, 2026-07-28)
# Assembles: gcc -nostdlib -no-pie test_sno_cell_4.s -o t4 && ./t4
# Subject "BlueGoldBirdFish":  POS(0) ARBNO('Bird' | 'Blue' | LEN(1)) $ OUTPUT RPOS(0)
# == the COMPLETE test_sno_1.c program, $ OUTPUT included.  $ is IMMEDIATE assignment:
# write-through at the close-twin (DESIGN-SN4-CELL-MACHINE.md COMMIT WALK rule), NOT a
# cell — it fires on EVERY yield of its subpattern, including the null first yield,
# and needs no rollback because write-through is the semantics (manual: occurs even
# if the whole match ultimately fails).  Under cells that is ONE print at the shy γ.
# ORACLE (sbl, quickscan AND fullscan agree, 2026-07-28): '' Blue BlueG ... ladder,
# then the full subject, then Success!.  ⚠ test_sno_1.c ITSELF DEVIATES from the
# oracle: no null yield (its ARBNO runs an instance before first γ) and doubled
# newlines.  This seed matches the ORACLE — the house rule — not the C deviation.
# Oracle:   sbl -b  →  text=BlueGoldBirdFish  len=16       (verified 2026-07-28)
# Expected: CELL4 MATCH start=0 end=16 text=BlueGoldBirdFish
#
# ═══════════════════════════════════════════════════════════════════════════════════
...
n2_arbno_carve:                         # recursion re-entry: r10 = start, r11 = outer sib
    ...
    mov     [rsp + U_LEN], eax
    mov     rbx, rsp                    # γ
    # ── $ OUTPUT — write-through at the close: print [aux, δ) + newline ──────────
    #    (r11 not live here: consumed into U_PREVSIB above, reloaded at after_P;
    #     syscall's rcx/r11 clobbers are therefore harmless at this site)
    mov     eax, 1
    mov     edi, 1
    lea     rsi, [r13 + r10]
    mov     edx, r14d
