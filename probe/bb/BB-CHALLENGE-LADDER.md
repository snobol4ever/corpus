...
⭐ **The "oracle" column moved 2026-08-28** (probe-consolidate-bb, LON-20260828 total conversion): each
bare filename below was a standalone `corpus/probe/bb/<name>.sno`; all are now entries (by the same name)
in `corpus/tests/snobol4/probe/bb.sno`/`.ref`, addressed `bb.sno#<name>` (family#name, the suite board
identity convention). All still oracle-verified GREEN, re-checked live during the move. Extract one
standalone for inspection: `python3 SCRIP/scripts/corpus_suite_harness.py extract corpus/tests/snobol4/probe/bb.sno corpus/tests/snobol4/probe/bb.ref <name> /tmp/<name>.sno`.

| # | pattern | challenge isolated | C | oracle (now `bb.sno#…`) | state |
|---|---|---|---|---|---|
| 1 | `POS(0) ARBNO('Bird'\|'Blue'\|LEN(1)) $ OUTPUT RPOS(0)` | ARBNO⊗ALT · shy-null · derived-vs-accumulated result · per-iteration arm index | `test_sno_1.c` | `bb.sno#t1` | ✅ GREEN |
| 4 | `POS(0) ARBNO(('Bird'\|'Blue'\|LEN(1)) . W) RPOS(0)` | **conditional capture INSIDE the body** — unbounded pending entries; push/pop/commit. The rung that reverted SCRIP ZW16. | `test_sno_4.c` | `bb.sno#c4`,`bb.sno#c4b` | ✅ GREEN |
| 5 | `POS(0) ARBNO(SPAN('0-9')\|LEN(1)) $ OUTPUT RPOS(0)` | **variable-extent arm** — β cannot restore the cursor arithmetically; first genuinely-needed per-instance saved δ | — | `bb.sno#c5` | oracle only |
| 6 | `POS(0) ARBNO(LEN(1)) $ OUTPUT FENCE 'X' RPOS(0)` | **FENCE backward-abort** — oracle proves ARBNO is never retried (one null yield, then fail) | — | `bb.sno#c6` | oracle only |
| 7 | `P = LEN(1); POS(0) ARBNO(*P) $ OUTPUT RPOS(0)` | **deferred/unevaluated** `*VAR` — which pattern arrives is decided at match time; per-iteration extent varies | — | `bb.sno#c7` | oracle only |
...
**Case 4 — CORRECTED 2026-08-04 (Lon ruling + `bb.sno#f1` witness, formerly `f1.sno`): the CAS MUST STAY A SEPARATE ARENA.** The claim below holds ONLY on the failure path; it is FALSE across a forward whack. WHACK-free on FENCE0/FENCE1 is MANDATORY (memory: O(activations)→O(depth)), and a pending `.` entry must SURVIVE that whack without committing at it (`bb.sno#f1`: capture behind a fence stays unset when the match later fails). Therefore the entry cannot live in the whacked region. What survives is SCOPE: the CAS becomes MATCH-scoped, not the process-wide 8MB pin. Original (failure-path-only) reasoning follows.
