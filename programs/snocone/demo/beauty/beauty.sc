...
// Mechanical translation of beauty.sno main00..main05 + mainErr1/2.
// Each label maps to its .sno counterpart; flow follows .sno verbatim:
//   :F(LBL)  →  if (~(EXPR)) { goto LBL; }
//   :S(LBL)  →  if (EXPR)    { goto LBL; }
//   :(LBL)   →  goto LBL;
// Body parts (Line=INPUT, Src=Src Line nl, Src ? *Parse, DIFFER(sno=Pop()),
// pp(sno), the OUTPUTs) preserved verbatim from .sno.
// `END` is SNOBOL4-reserved; the .sc port uses `mainEnd` instead.
...
if (~DIFFER(sno = Pop())) { goto mainErr2; }
//                   pp(sno)                                              :(main01)
pp(sno);
goto main01;
...
//                   OUTPUT         =  Src                                :(END)
OUTPUT = Src;
// END (SNOBOL4-reserved; trivial stmt binds the mainEnd label)
mainEnd: doDebug = doDebug;
