	gl_options = ""
	if (.A :: 'A') {
		# here if running under Macro SNOBOL4 (ie; SNOBOL4+, C-MAINBOL)
...
#	Epilogue
	INPUT(.inf,-2,'/v/lib/snocone/epilogue')
	while (line = inf)
		outf = line
	ENDFILE(-2)
...
procedure getline() x, file, del, dir {
	do {
		...
		gl_close()
	} while (gl_index >= gl_first || gl_nextfile());
