...
// Snocone: whitespace is spaces, tabs, newlines, and comments — all equivalent.
// No line structure, no continuation lines (ARCH-SNOCONE.md).
White    = SPAN(' ' tab nl '\r\f') | '//' BREAK(nl) | Block;
Gray     = ARBNO(*White);
$' '     = Gray;
/*--------------------------------------------------------------------------------------------------------------------*/
...
kw_for      = ('for'      kw_tail);
// NonKwId — identifier that is NOT a reserved keyword.
// Prevents 'function', 'if', 'while', etc. from being parsed as E_VAR in Expr17.
NonKwId  = (*Id . captured_id)
           FENCE(~*kw_if ~*kw_while ~*kw_do ~*kw_else ~*kw_function
                 ~*kw_return ~*kw_freturn ~*kw_nreturn ~*kw_goto ~*kw_for);
/*====================================================================================================================*/
...
// Gray (horizontal-only) for '(' so a newline between id and '(' is NOT a call.
ArgFirst = ( *Expr0 nInc() );
ArgRest  = ( $','   *Expr0 nInc() );
CallArgs = ( ArgFirst ARBNO(ArgRest) | epsilon );
Call     = ( (*NonKwId . captured_call_name)
