...
begin
  ...
  begin
    var _lcp: ctp; _found: boolean;
    _lcp := display[0].fname; _found := false;
    while _lcp <> nil do
      if _lcp^.name = 'writeln ' then begin _found := true; _lcp := nil end
      else if _lcp^.name < 'writeln ' then _lcp := _lcp^.rlink
      else _lcp := _lcp^.llink;
    if _found then write(output,'[PRECHECK=OK]') else write(output,'[PRECHECK=FAIL]')
  end;
  top := 1; level := 1;
  with display[1] do
    begin fname := nil; flabel := nil; occur := blck end;
